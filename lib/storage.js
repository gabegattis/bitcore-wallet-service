...
Storage.prototype.removeWalletTransactionsAtBlockHeight = function(blockHeight, callback) {
  this.db.collection(collections.WALLET_TRANSACTIONS).remove({blockHeight: blockHeight}, callback);
};

Storage.prototype.getLatestBlock = function(callback) {
  /**
  ...
  */
  this.db.collection(collections.BLOCKS).find().sort({height: -1}).limit(1).toArray(function(err, blocks) {
    if (err) {
      return callback(err);
    }
...
Storage.prototype.deleteBlock = function(blockHeight, callback) {
  // cannot use .remove() with capped collection
  this.db.collection(collections.BLOCKS).remove({height: blockHeight}, callback);
};

Storage.prototype.finishBlock = function(blockHeight, callback) {
  this.db.collection(collections.BLOCKS).update({height: blockHeight},
...
Storage.prototype.startRemovingBlock = function(blockHeight, callback) {
...
Storage.prototype.addBlock = function(block, callback) {
