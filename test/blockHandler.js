...
var ADDRESS_OBJECTS = [
  Address.create({
    address: '15urYnyeJe3gwbGJ74wcX89Tz7ZtsFDVew', walletId: '0',
    isChange: false, path: '', publicKeys: '', type: Constants.SCRIPT_TYPES.P2PKH
  }),
  Address.create({
    address: '1LBirB1dBCoXb7btHoU6Fxc46dkSegiqFN', walletId: '0',
    isChange: false, path: '', publicKeys: '', type: Constants.SCRIPT_TYPES.P2PKH
  }),
  Address.create({
    address: '38ZQf7oz9B25LUsRHaguWXSeV94MMDSMvK', walletId: '1',
    isChange: false, path: '', publicKeys: '', type: Constants.SCRIPT_TYPES.P2SH
  }),
  Address.create({
    address: '13AfVbCHNa2mJp7UkpNFH2xioURmjBCLKW', walletId: '2',
    isChange: false, path: '', publicKeys: '', type: Constants.SCRIPT_TYPES.P2PKH
  }),
  Address.create({
    address: '1Bc9cXVZo2D3dG1YB7wqEBdE3H34ovheRm', walletId: '3',
    isChange: false, path: '', publicKeys: '', type: Constants.SCRIPT_TYPES.P2PKH
  }),
  Address.create({
    address: '1TrumpveJrAYhsmuU2FdeMy48wuvQswKn', walletId: '4',
    isChange: true, path: '', publicKeys: '', type: Constants.SCRIPT_TYPES.P2PKH
  })
];
...
describe('BlockHandler', function() {
  ...
  describe('handleAddBlock', function() {
    it('should handle fetchAddressesForBlockHandler error', function(done) {
      var sandbox = sinon.sandbox.create();

      var fa = sandbox.stub(Storage.prototype, 'fetchAddressesForBlockHandler', function(addresses, callback) {
        callback(new Error('this is an error'));
      });

      var block = bitcore.Block.fromString(RAW_BLOCK_HEX);
      block.height = 406016;
      ...
      blockHandler.handleAddBlock(block, function(err) {
        ...
        fa.callCount.should.equal(1);
        sandbox.restore();
        done();
      });
    });

    it('should handle createWalletTransaction error', function(done) {
      var sandbox = sinon.sandbox.create();

      var fa = sandbox.stub(Storage.prototype, 'fetchAddressesForBlockHandler', function(addresses, callback) {
        callback(null, ADDRESS_OBJECTS);
      });
      ...
      blockHandler.handleAddBlock(block, function(err) {
        ...
        err.message.should.equal('this is an error');
        fa.callCount.should.equal(1);
        cw.callCount.should.equal(1);
        sandbox.restore();
        done();
      });
    });

    it('should add walletTransactions for block 406016', function(done) {
      var sandbox = sinon.sandbox.create();
      ...
      var sw = sandbox.stub(Storage.prototype, 'storeWalletTransaction', function(walletTransaction, callback) {
        callback();
      });
      ...
      blockHandler.handleAddBlock(block, function(err) {
        ...
        sw.callCount.should.equal(9);
        sw.args[0][0].should.deep.equal(WalletTransaction.create({
